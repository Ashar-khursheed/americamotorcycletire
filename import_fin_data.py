...
def main():
    ...
    print("Connected to MySQL database 'americamotor'. Loading excel...", flush=True)

    wb = openpyxl.load_workbook('f:/americamotorcycletire/fin.xlsx', data_only=True)
    ...
    # Clear existing demo products & fitments for fresh full dataset import
    print("Clearing old demo products and fitments...", flush=True)
    ...
    product_id_map = {} # product_name_lower -> product_id
    used_slugs = set()
    ...
    # Process Fitments from Sheet 3: Size Breakdown & Fitments
    ...
    print(f"Processing size fitments from Sheet 3 ({len(rows3)-1} rows)...", flush=True)
    ...
    # Also build fitments from Sheet 1 for products without Sheet 3 fitments
    ...
    print(f"Executing batch insert for {len(fitment_batch)} fitments...", flush=True)
    ...
    for i in range(0, len(fitment_batch), chunk_size):
        ...
        db.commit()
        print(f"Inserted fitment chunk {i} to {i+len(chunk)}...", flush=True)

    print(f"SUCCESS! Total Products: {len(product_id_map)}, Total Fitments: {len(fitment_batch)}", flush=True)
